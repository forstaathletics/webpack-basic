...
MaterialLayout.prototype.Constant_ = {
    ...
    MENU_ICON: String.fromCharCode(0xE5D2),
...
MaterialLayout.prototype.init = function () {
    if (this.element_) {
        ...
        // If the first child is an innter-container, use it's children
        if (directChildren[0] && directChildren[0].classList.contains(this.CssClasses_.INNER_CONTAINER)) {
          directChildren = directChildren[0].childNodes
        }
        ...
        if (this.drawer_) {
            ...
            // Only append the obfuscator if it's not already present.
        ...
        // Allow for a manual inner container, good for React.
        // We look for an existing inner-container, then for any at all.
        // If we don't find one, we create one.

        var innerContainer;
        if (this.element_.firstChild && this.element_.firstChild.classList.contains(this.CssClasses_.INNER_CONTAINER)) {
          innerContainer = this.element_.firstChild;
        } else {
          innerContainer = document.querySelector('.' + this.CssClasses_.INNER_CONTAINER);
        }
